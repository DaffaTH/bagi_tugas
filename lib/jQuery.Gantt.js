...
(function ($, undefined) {
    ...
    $.fn.gantt = function (options) {
        ...
        var settings = {
            ...
            onRender: function(core, element) { return; },
            scrollToToday: true
        };
        ...
        */
        ...
        var core = {
            ...
            init: function (element) {
                ...
                // element.dateStart = tools.getMinDate(element); <-- EDIT
                // element.dateEnd = tools.getMaxDate(element); <-- EDIT
                element.dateStart = settings.dateStart || tools.getMinDate(element);
                element.dateEnd = settings.dateEnd || tools.getMaxDate(element);
            ...
            render: function (element) {
                ...
                settings.onRender(core, element);
            },
            ...
            dataPanel: function (element, width) {
                ...
                dataPanel.click(function (e) {
                    ...
                    */
                    ...
                    // console.info([e.pageX, datapanel.offset().top, corrY, datapanel.offset().top + corrY], col);
                    if (col===null) col = document.elementFromPoint(e.pageX, 70); // <- EDIT
                    ...
                    ganttNavigate = $('<div class="navigate" />')
                        .append($('<div class="nav-slider" />')
                            .append($('<div class="nav-slider-left" />')
                                .append($('<button type="button" class="nav-link nav-page-back" title="Halaman Sebelumnya"/>')
                                    .html('&lt;')
                                    .click(function () {
                                ...
                                .append($('<button type="button" class="nav-link nav-page-next" title="Halaman Selanjutnya"/>')
                                    .html('&gt;')
                                    .click(function () {
                                        core.navigatePage(element, 1);
                                    }))
                                .append($('<button type="button" class="nav-link nav-now" title="Hari Ini"/>')
                                    .html('&#9679;')
                                    .click(function () {
                                        core.navigateTo(element, 'now');
                                    }))
                                .append($('<button type="button" class="nav-link nav-prev-week" title="Bulan Lalu"/>')
                                    .html('&lt;&lt;')
                                    .click(function () {
                                ...
                                .append($('<button type="button" class="nav-link nav-prev-day" title="Minggu Lalu"/>')
                                    .html('&lt;')
                                    .click(function () {
                                        if (settings.scale === 'hours') {
                            ...
                            .append($('<div class="nav-slider-content" />')
                                    .append($('<input type="range" class="custom-range nav-slider-range" value="0">')
                                            .on('input change', function() { core.sliderRange(element, $(this).val()) })
                                        )
                            ...
                            .append($('<div class="nav-slider-right" />')
                                .append($('<button type="button" class="nav-link nav-next-day" title="Minggu Depan"/>')
                                    .html('&gt;')
                                    .click(function () {
                                        if (settings.scale === 'hours') {
                                            core.navigateTo(element, tools.getCellSize() * -4);
                            ...
                            .append($('<button type="button" class="nav-link nav-next-week" title="Bulan Depan"/>')
                                    .html('&gt;&gt;')
                                    .click(function () {
                                        if (settings.scale === 'hours') {
                                            core.navigateTo(element, tools.getCellSize() * -8);
            ...
            createProgressBar: function (days, cls, desc, label, dataObj) {
                ...
                if (settings.popover) {
                    bar.popover(settings.popover(dataObj));
                }
            ...
            markNow: function (element) {
                switch (settings.scale) {
                    ...
                    default:
                        ...
                        $(element).find('#dh-'+moment().startOf('day').valueOf()+',#dw-'+moment().startOf('day').valueOf()).removeClass('wd').addClass('today'); // <- EDIT
                        break;
                }
            },
                        ...
                        $.each(entry.values, function (j, day) {
                            ...
                            switch (settings.scale) {
                                ...
                                default:
                                    ...
                                    // _bar.css({ 'top': top, 'left': Math.floor(cFrom) });
                                    _bar.css({ 'top': top, 'left': typeof cFrom==='undefined'? -24 : Math.floor(cFrom) }); // <-- EDIT
            ...
            navigateTo: function (element, val) {
                ...
                switch (val) {
                    ...
                    default:
                        ...
                        $dataPanel.animate({ "margin-left": Math.min(0, Math.max(max_left, val)) + "px" }, "fast", shift);
                        element.scrollNavigation.panelMargin = val;
                        break;
                }
            ...
            navigatePage: function (element, val) {
                // if ((element.pageNum + val) >= 0 && (element.pageNum + val) < Math.ceil(element.rowsNum / settings.itemsPerPage)) {
                    core.waitToggle(element, true, function () {
                        // element.pageNum += val;
                        element.pageNum = (element.pageNum+val+element.pageCount) % element.pageCount; // <-- EDIT
            ...
            wheelScroll: function (element, e) {
                if ($(e.target).closest('.rightPanel').length===0) return true;
                e.preventDefault(); // e is a jQuery Event

                // attempts to normalize scroll wheel velocity
                var delta = ( 'wheelDelta' in e.originalEvent ? - 1/120 * e.originalEvent.wheelDelta :
            ...
            // Move chart via slider range control
            sliderRange: function (element, val) {
                let $rightPanel = $(element).find(".fn-gantt .rightPanel");
                let $dataPanel = $rightPanel.find(".dataPanel");
                let mLeft = $rightPanel.width() - $dataPanel.width();
                $dataPanel.css("margin-left", (val/100*mLeft).toFixed(0) + "px");
            },
            ...
            synchronizeScroller: function (element) {
                if (settings.navigate === "scroll") {
                    ...
                    let $dataPanel = $rightPanel.find(".dataPanel");
                    let mLeft = $rightPanel.width() - $dataPanel.width(),
                        hPos = $dataPanel.css("margin-left") ? $dataPanel.css("margin-left").replace("px", "") : 0;
                    $(element).find(".nav-slider-range").val((hPos/mLeft*100).toFixed(0));
                }
                // if (settings.navigate === "scroll") {
                //     var $rightPanel = $(element).find(".fn-gantt .rightPanel");
                //     var $dataPanel = $rightPanel.find(".dataPanel");
