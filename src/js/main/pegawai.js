var pegawaiByNip = pegawai.reduce((obj, { nipbps, nip:a, ...rest }) => (obj[nipbps] = { nip: nip(a).print(), ...rest }, obj), {});

const ME = pegawaiByNip[NIP] || { nama: NIP, username: NIP };
...
const modalViewPegawai = nipbps => {
	...
	let { nip, username, nama, urlfoto, satker, pangkat, golongan, seksi, jabatan, level, login_terakhir } = pegawaiByNip[nipbps];
	modal({
		title: nama,
		body: `
...
const modalEditPegawai = nipbps => {
	let { nip, urlfoto, seksi, urutan, nama, username, jabatan, golongan, pangkat, level='6' } = pegawaiByNip[nipbps] || {};
	const getInputField = (label, { name, value, maxlength=64, inputmode, placeholder }) => `
...
const modalEditPegawaiProcessData = () => {
	...
	if (!data.urutan) { $('#modal [name="urutan"]').closest('[class^="col-"]').addClass('has-error'); ok=0; }
	if (!data.nama) { $('#modal [name="nama"]').closest('[class^="col-"]').addClass('has-error'); ok=0; }
	if (!/\d{9}/.test(data.nipbps)) { $('#modal [name="nipbps"]').closest('[class^="col-"]').addClass('has-error'); ok=0; }
	if (!/\d{18}/.test(data.nip)) { $('#modal [name="nip"]').closest('[class^="col-"]').addClass('has-error'); ok=0; }
	if (!data.username) { $('#modal [name="username"]').closest('[class^="col-"]').addClass('has-error'); ok=0; }
	if (!data.jabatan) { $('#modal [name="jabatan"]').closest('[class^="col-"]').addClass('has-error'); ok=0; }
	if (!data.golongan) { $('#modal [name="golongan"]').closest('[class^="col-"]').addClass('has-error'); ok=0; }
	if (!data.pangkat) { $('#modal [name="pangkat"]').closest('[class^="col-"]').addClass('has-error'); ok=0; }
	if (!data.seksi) { $('#modal [name="seksi"]').closest('[class^="col-"]').addClass('has-error'); ok=0; }
	if (ok) return data;
}

const modalDeletePegawai = nipbps => {
	let { nama, jabatan } = pegawaiByNip[nipbps] || {};
...
const modalEditMitra = ({ id='', kec='', nama='', asal='', no_hp='', nik='', no_rek='', npwp='' }) => {
	if (id && !nama) {
	...
	modal({
		...
		data: {
			'shown.bs.modal': () => {
				$('#modal [name="kec"]').autocomplete({ source: autoc.kec });
...
const modalEditMitraProcessData = () => {
	...
	if (!data.kec) { $('#modal [name="kec"]').closest('[class^="col-"]').addClass('has-error'); ok=0; }
	if (!data.nama) { $('#modal [name="nama"]').closest('[class^="col-"]').addClass('has-error'); ok=0; }
	if (ok) return data;
}

const modalDeleteMitra = id => {
	let { nama, kec } = mitra.find(a => a.id==id);
...
$(()=>{
	...
	pegawaiTable = $('#table-pegawai').DataTable(LV<5? {
		...
		columns: [{
			...
			title: 'Nama / NIP',
			data: null,
			render: ({ nipbps, nip, nama }) => `<div class="hover:underline cur-p text-primary" onclick="modalViewPegawai(${nipbps})">${nama}</div><div class="text-muted fz-10">${nip}</div>`
		},{
			title: 'Email',
			data: 'username',
			...
			data: null,
			render: ({ pangkat, golongan }) => `<div>${pangkat}</div><div class="text-muted fz-10">${golongan}</div>`
		},{
			title: 'Peran',
			data: 'level',
			...
			searchable: false,
			sortable: false,
			render: ({ nipbps }) => `<i class="fas fa-pen fa-fw py-1 cur-p fz-16 text-primary mr-2" title="Edit" onclick="modalEditPegawai(${nipbps})"></i><i class="fas fa-trash fa-fw py-1 cur-p fz-16 text-danger" title="Hapus" onclick="modalDeletePegawai(${nipbps})"></i>`
	...
	mitraTable = $('#table-mitra').DataTable(LV<5? {
		...
		columns: [{
			...
			title: 'Asal',
			data: null,
			render: ({ kec, asal }) => `${kec}<div class="text-muted fz-11">${asal}</div>`
		},{
			title: 'No. Hp',
			data: 'no_hp',
		},{
			...
			searchable: false,
			sortable: false,
			render: ({ id }) => `<i class="fas fa-pen fa-fw py-1 cur-p fz-16 text-primary mr-2" title="Edit" onclick="modalEditMitra({id:${id}})"></i><i class="fas fa-trash fa-fw py-1 cur-p fz-16 text-danger" title="Hapus" onclick="modalDeleteMitra(${id})"></i>`
		}],
	} : {
		data: mitra,
		rowId: 'id',
