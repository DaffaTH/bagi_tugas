$(()=>{
	...
	.on('click', '.open-modal-kwitansi-btn', function() {
		let { id, no, tgl, pws, pelaksana, anggota, tempat, tgl_mulai, tgl_akhir } = $modal.data(),
			...
			data: {
				'show.bs.modal': () => {
					...
					$('#modal .uang').toArray().forEach(function(field) {
						new Cleave(field, { numeral: true, numericOnly: true, numeralDecimalScale: 0, numeralDecimalMark: ',', delimiter: '.' });
					});
				},
			},
	...
	.on('click', '.submit-kwitansi-btn', function() {
		...
		let { no, tgl, pws, tujuan } = $modal.data(),
		...
		let data = {
			glob: {
				...
				tgl_bayar,
				bendahara: { nama: bendahara.nama, nip: bendahara.nip },

			},
			data: $('#modal [name="pelaksana[]"]').val().map(a => {
				let { nama, nip, jabatan, golongan, pangkat } = getOrang(a);
				return { nama, nip, golongan };
			}),
		}
