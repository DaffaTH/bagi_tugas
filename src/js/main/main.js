...
const route = (href, push=true) => {
	...
	document.documentElement.classList.remove('nav_open'); $navToggler.removeClass('toggled'); nav_open = 0; // Minimize Sidebar

	document.title = $route.data('title');

	if (push) window.history.pushState(null, document.title, urlBase+href);

	if (href==='' && ganttNotRendered) renderGantt({ pegawai, surat, libur, tglGantt, ganttItems });
}
...
const getOrang = id => {
	...
	if (orang) return { jabatan: 'Mitra BPS', nip: '-', golongan: '-', pangkat: '-', ...orang };
...
function Nip(xxx) { this.nip = xxx; }
const nip = xxx => new Nip(xxx);
nip.fn = Nip.prototype = {
	print: function() { return this.nip.substr(0,8) + ' ' + this.nip.substr(8,6) + ' ' + this.nip.substr(14,1) + ' ' + this.nip.substr(15,3); },
}
...
const selectpickerPegawai = ({ name='pegawai', selected, title='Pilih pegawai...' }) => `
...
const selectpickerPegawaiDanMitra = ({ name='pelaksana', selected, title='Pilih pelaksana...', disabled=false }) => `
...
$(()=>{
	...
	.on('click', '.submit-upload-template-btn', () => {
		...
		$modal.find('[name]').each((i,form) => {
			...
			else { $form.parent().addClass('has-error'); ok=0; }
		});
		if (ok) {
...
const refresh = () => {
	if (urlPath('')) renderGantt({ pegawai, surat, libur, tglGantt, ganttItems });
...
const refreshPegawai = () => {
	...
	pegawaiByNip = pegawai.reduce((obj, { nipbps, nip:a, ...rest }) => (obj[nipbps] = { nip: nip(a).print(), ...rest }, obj), {});
