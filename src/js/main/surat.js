const JENIS = SATKER.substr(2)==='00'?
	['Lainnya','Bagian Tata Usaha','Bidang Statistik Sosial','Bidang Statistik Produksi','Bidang Statistik Distribusi','Bidang NWAS','Bidang IPDS'] :
	['Lainnya','Subbagian Tata Usaha','Seksi Statistik Sosial','Seksi Statistik Produksi','Seksi Statistik Distribusi','Seksi NWAS','Seksi IPDS'];
...
const sortNoSpd = ({no_spd:b=0}, {no_spd:a=0}) => {
	let firstInt = parseInt(a, 10);
	let secondInt = parseInt(b, 10);
	if (firstInt != secondInt) return firstInt - secondInt;
	else return 1000*a.charCodeAt(a.length-2)+a.charCodeAt(a.length-1) - 1000*b.charCodeAt(b.length-2)+b.charCodeAt(b.length-1);
...
const sortTgl = ({tgl:a}, {tgl:b}) => moment(a, f0).isBefore(moment(b, f0), 'day')? 1 : (moment(a, f0).isAfter(moment(b, f0), 'day')? -1 : 0);
...
const renderGantt = ({ pegawai, surat, libur, tglGantt, ganttItems=999 }) => {
	$gantt.gantt({
		...
		onItemClick: (data) => { console.log(data); modalViewSurat(data); },
		onAddClick: (dt, rowId) => {
			...
			if (rowId) {
				if (LV<6) {
					if (LV>4) {
						if (date.isBefore(PREVWEEK, 'day')) return notif('Pembuatan surat tugas sebelum minggu lalu hanya dapat dilakukan oleh <span class="fw-600">Administrator</span>', 'danger', 0, { delay: 1000 });
						else if (date.isAfter(moment(NEXTMONTH, f1), 'day')) return notif('Pembuatan surat tugas di atas sebulan yang akan datang hanya dapat dilakukan oleh <span class="fw-600">Administrator</span>', 'danger', 0, { delay: 1000 });
					}
					modalEditSurat({ pelaksana: rowId, tgl: date.format(f1) });
				}
			}
...
const mapPegawaiSurat = ({ nipbps, nip, username, nama, urlfoto }, surat, dateStart) => ({
	id: nipbps,
	name: surat.find(({ tgl_mulai, tgl_akhir, pelaksana, no_spd }) => IS_WEEKEND || pelaksana.includes(nipbps) && moment().isBetween(tgl_mulai, tgl_akhir, 'day', '[]') && no_spd)? '<i class="fa-fw fas fa-fingerprint text-lightgray"></i>' : '<i class="fa-fw fas fa-fingerprint text-success"></i>',
	desc: nama,
	values: surat
...
const modalEditTanggal = () => {
	modal({
		...
		data: {
			'shown.bs.modal': () => {
				$('#modal .input-daterange').datepicker({ endDate: moment().add(2, 'months').format(f1) });
...
const modalRekap = tgl_x => {
	let tugas_tgl_x = surat.filter(({ pelaksana, tgl_mulai, tgl_akhir }) => moment(tgl_x).isBetween(tgl_mulai, tgl_akhir, 'day', '[]') && !pelaksana.split(',').every(a=>a.includes('m')));
	let tgl_x_prev = moment(tgl_x).subtract(1, 'days'),
		tgl_x_next = moment(tgl_x).add(1, 'days');
	modal({
		title: tgl_x.format('dddd, D MMMM YYYY'),
		body: tugas_tgl_x.length? tugas_tgl_x.map(({ pelaksana, no_spd, tempat }) => {
			let anggota = [];
			...
			`
		}).join('<div class="pb-2"></div>') :
		'<div class="text-center py-3"><i class="fas fa-ban fa-5x text-lightgray"></i><div class="mt-3 text-gray">Tidak ada tugas</div></div>',
		primaryBtn: 'd-none',
		secondaryBtn: 'd-none',
		misc: `
		...
		`,
		data: {
			'shown.bs.modal': () => { $('[data-toggle="tooltip"]').tooltip() }
		},
	});
}

const getAva = ({ urlfoto, nama }, tooltip=false) => urlfoto? `<div class="avatar" title="${nama}" ${tooltip? 'data-toggle="tooltip" data-trigger="hover" data-placement="right"' : ''}><img src="https://community.bps.go.id/images/avatar/${urlfoto}" class="avatar-img rounded-circle opos-top border border-white"></div>` : `<div class="avatar"><span class="avatar-title rounded-circle border border-white bg-secondary">${(nama||' ').charAt(0)}</span></div>`;
...
const modalViewSurat = ({ id, no='', no_spd='', pws, tgl, ttd, pelaksana, tujuan='', tempat='', angkutan='', tgl_mulai, tgl_akhir, jenis, dibuat_oleh, terakhir_update }) => {
	if (id && !no) {
	...
	modal({
		...
		`,
		data: { id, no, no_spd, pws, tgl, ttd, pelaksana, anggota, tujuan, tempat, angkutan, tgl_mulai, tgl_akhir, jenis, dibuat_oleh, terakhir_update },
	});
}

const modalEditSurat = ({ id, no='', no_spd='', pws, tgl, ttd, pelaksana, anggota=[], tujuan='', tempat='', angkutan='', tgl_mulai, tgl_akhir, jenis, dibuat_oleh, terakhir_update }) => {
	...
	modal({
		...
		data: {
			'shown.bs.modal': () => {
				$('#modal .toggle-pws').change(function() { $('#modal [name="no"]').val('') });
				...
				$('#modal [name="tgl"]').datepicker(LV>4? { startDate: PREVWEEK.format(f1), endDate: moment(tglGantt[1]).format(f1) } : { endDate: moment(tglGantt[1]).format(f1) });
				$('#modal .input-daterange').datepicker(LV>4? { startDate: PREVWEEK.format(f1) } : { endDate: moment().add(2, 'months').format(f1) });
				$('#modal [name="tujuan"]').autocomplete({ source: autoc.tjn });
				$('#modal [name="tempat"]').autocomplete({ source: autoc.tmp });
				$('#modal [name="ttd"]').change(function() { $('#ttd-setting-container').toggle($(this).val()!==setting.kepala) }).change();
			},
			'show.bs.modal': () => {
...
const modalEditSuratProcessData = () => {
	...
	$modal.find('[name]').each((i,form) => {
		let $form = $(form),
			name = $form.attr('name'),
			val = $form.val();
		if (name==='pws') { data['pws'] = $form.is(':checked')? 1 : 0 }
		else if ( ((data[name]=val)=='' && $form.data('req')) || (name==='no_spd' && val==='' && !$form.attr('disabled')) || (name==='pelaksana' && !val) || (name==='tgl_akhir' && data['tgl_mulai']==='') )
			{ $form.closest('[class^="col-"]').addClass('has-error'); ok=0; }
		else $form.closest('[class^="col-"]').removeClass('has-error');
	});
	if (moment(data.tgl_mulai, f1).isBefore(moment(data.tgl, f1), 'day')) { $('#modal [name="tgl"]').closest('[class^="col-"]').addClass('has-error'); ok=0; }
	if (ok) {
...
const modalDeleteSurat = ({ id, no, no_spd, pws, tgl }) => {
	modal({
		data: { id },
	...
	$.ajax({
		...
		success: res => {
			...
			if (res.success) {
				...
				let { tujuan, tgl_mulai, tgl_akhir } = res.data[0];
...
const downloadSelectedSurat = () => {
	let suratFiltered = surat.filter(a => moment(a.tgl_mulai).isSameOrAfter(tglGantt[0], 'day')).reverse(),
		data = {
		glob,
		data: suratTable.rows({ selected: true })[0].map(a => prepareDataToPost(suratFiltered[a]))
	};
	console.info(data);
...
const exportSurat = () => {
	notif('Mengekspor ke Excel...', 'wait', false, { delay: 500 });
	const getAnggota = (a) => {
	...
	let wb = XLSX.utils.book_new(),
		ws = XLSX.utils.aoa_to_sheet([
			...
			...surat.reverse().map(({ no='', no_spd='', pws, tgl, ttd, pelaksana, tujuan='', tempat='', angkutan='', tgl_mulai, tgl_akhir, jenis, dibuat_oleh, terakhir_update }) => {
				let { nip, jabatan, golongan='-', pangkat='-' } = getOrang(pelaksana.split(',')[0]);
				return [
...
const $daftarTugas = { st: $('#daftar-st'), pws: $('#daftar-pws'), spd: $('#daftar-spd') };
...
const generateDaftarTugas = (arraySurat, jenis) => {
	...
	arraySurat.forEach((a,i) => { html += generateTimeline({ id: a.id, no: getNomorSurat(a[no], a.tgl, jenis), tgl: i&&arraySurat[i-1].tgl===a.tgl? false : moment(a.tgl, f0).format('D MMM'), pelaksana: a.pelaksana, jenis: a.jenis }, i) });
	html += '</div></div></div>';
	$daftarTugas[jenis]
	.html(html);
}

const generateTimeline = ({ id, no, tgl, pelaksana, jenis }, notFirst) => {
	no = no==='-'?
...
$(()=>{
	...
	$('.buat-st-btn').click(() => { modalEditSurat({ pelaksana: '', no: generateNomorSurat('st') }) });
	$('.buat-pws-btn').click(() => { modalEditSurat({ pws: '1', pelaksana: '', no: generateNomorSurat('pws') }) });
	...
	suratTable = $('#table-surat').DataTable({
		...
		columns: [{
			...
			title: 'No. ST',
			data: null,
			render: ({ no, tgl, pws }) => getNomorSurat(no, tgl, pws==1? 'pws' : null)
		},{
			title: 'No. SPD',
			data: null,
			render: ({ no_spd, tgl }) => no_spd!=null&&no_spd!='-'? getNomorSurat(no_spd, tgl, 'spd') : '<span class="text-muted">-</span>'
		},{
			...
			searchable: false,
			sortable: false,
			render: ({ id }) => `<i class="fas fa-info-circle fa-fw py-1 cur-p fz-16 text-primary" title="Edit" onclick="modalViewSurat({id:${id}})"></i>`
		}],
	});

	$('#table-surat').on('draw.dt', function() { $('#table-surat th:nth-child(2), #table-surat td:nth-child(2)').addClass('without-st-pws-hidden') });

	suratTable.on('select deselect', function() {
		let n = suratTable.rows({ selected: true })[0].length;
...
const generateCalendar = () => {
	$(()=>{
		suratCalendar = new Calendar('#calendar', {
			...
			dataSource: surat
			.filter(({ pelaksana }) => pelaksana.includes(NIP))
			.map(({ id, no, no_spd, pws, tgl, tujuan, tempat, tgl_mulai, tgl_akhir, jenis }) => ({
				startDate: moment(tgl_mulai).toDate(),
				endDate: moment(tgl_akhir).toDate(),
				id, no, no_spd, pws, tgl, tujuan, tempat, tgl_mulai, tgl_akhir, jenis,
				color: COLOR[jenis],
			})),
			mouseOnDay: function({ element, events }) {
				if (events.length) {
					let { id, no, no_spd, pws, tgl, tujuan, tempat, tgl_mulai, tgl_akhir, jenis } = events[0];
					$(element).popover({ 
						trigger: 'manual',
						container: 'body',
						placement: 'bottom',
			...
			mouseOutDay: function({ element, events }) {
				if (events.length) {
...
const prepareDataToPost = ({ id, no, no_spd, pws, tgl, ttd, pelaksana, anggota, tujuan, tempat, angkutan, tgl_mulai, tgl_akhir, jenis, dibuat_oleh, terakhir_update }) => {
	if (!anggota) {
		anggota = pelaksana.split(',');
		pelaksana = anggota.shift();
	}
	let { nama, nip, jabatan, golongan, pangkat } = getOrang(pelaksana);
