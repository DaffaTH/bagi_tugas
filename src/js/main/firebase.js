...
if (typeof firebase !== 'undefined') {
	...
	dbBagitugas.onSnapshot((snap, a) => {
		if (initialLoad) {
			...
			let { user='', key='', msg='' } = snap.data();
			if (user && key!==KEY && msg) {
				if ($modal.hasClass('show')) modalHiddenAction = () => { announced(msg) };
				else announced(msg);
			}
		}
	});
...
function announced(msg) {
	$.ajax({
		...
		success: res => {
			...
			else notif('Gagal sinkronisasi data', 'danger', 1, { delay: 1000 });
		},
		error: e => {
			console.warn(e.status, e.statusText);
			if (window.location.hostname==='localhost') console.error(e.responseText);
			notif('Gagal sinkronisasi data', 'danger', 1, { delay: 1000 });
		}
	});
	notif(msg, 'wait', 0, { delay: 2000 }, { icon: 'icon-refresh' });
}
